...
HAL_StatusTypeDef HAL_ETH_Start_IT(ETH_HandleTypeDef* heth)
{
    ...
    */
        __HAL_ETH_DMA_ENABLE_IT(
            heth, (ETH_DMACIER_NIE | ETH_DMACIER_RIE | ETH_DMACIER_TIE |
...
#ifdef HAL_ETH_USE_PTP
                /* Handle Ptp  */
                heth->txPtpCallback(dmatxdesclist->PacketAddress[idx],
                                    timestamp);
#endif /* HAL_ETH_USE_PTP */
                /* Release the packet.  */
...
#ifdef HAL_ETH_USE_PTP
                /* Handle Ptp  */
                HAL_ETH_TxPtpCallback(dmatxdesclist->PacketAddress[idx],
                                      timestamp);
#endif /* HAL_ETH_USE_PTP */
                /* Release the packet.  */
        ...
        if (__HAL_ETH_DMA_GET_IT_SOURCE(heth, ETH_DMACIER_AIE))
        {
            ...
            else
            {
                ...
                __HAL_ETH_DMA_CLEAR_IT(
                    heth, (ETH_DMACSR_CDE | ETH_DMACSR_ETI | ETH_DMACSR_RWT |
                           ETH_DMACSR_RBU | ETH_DMACSR_AIS));
            }
#if (USE_HAL_ETH_REGISTER_CALLBACKS == 1)
...
HAL_StatusTypeDef HAL_ETH_SetMACFilterConfig(
    ETH_HandleTypeDef* heth, const ETH_MACFilterConfigTypeDef* pFilterConfig)
{
    ...
    filterconfig =
        ((uint32_t)pFilterConfig->PromiscuousMode |
         ...
         ((uint32_t)((pFilterConfig->BroadcastFilter == DISABLE) ? 1U : 0U)
          << 5) |
...
#ifdef HAL_ETH_USE_PTP
    heth->txPtpCallback =
        HAL_ETH_TxPtpCallback; /* Legacy weak TxPtpCallback   */
#endif                         /* HAL_ETH_USE_PTP */
