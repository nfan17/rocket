...
#include "stm32h5xx_hal_def.h"
  ...
  */
    typedef struct
    {
        ...
        uint32_t
            PLLN; /*!< PLLN: Multiplication factor for PLL1 VCO output clock.
        ...
        uint32_t
            PLLFRACN; /*!< PLLFRACN: Specifies Fractional Part Of The Multiplication Factor for
  ...
  */
    typedef struct
    {
        ...
        uint32_t
            CSICalibrationValue; /*!< The calibration trimming value (default is RCC_CSICALIBRATION_DEFAULT).
...
#define RCC_HSE_BYPASS          \
    ((uint32_t)(RCC_CR_HSEBYP | \
                RCC_CR_HSEON)) /*!< External Analog clock source for HSE clock */
...
#define RCC_ALL                                                               \
    (RCC_HSI | RCC_HSE | RCC_CSI | RCC_LSI | RCC_LSE | RCC_HSI48 |            \
     RCC_SYSCLK | RCC_PRESC | RCC_PLL1 | RCC_PLL2 | RCC_PLL3 | RCC_CKPERSEL | \
     RCC_RMVF)
/**
  * @}
  */
...
#define IS_RCC_PLL1_VCORGE_VALUE(VALUE)     \
    (((VALUE) == RCC_PLL1_VCORANGE_WIDE) || \
     ((VALUE) == RCC_PLL1_VCORANGE_MEDIUM))
...
#include "stm32h5xx_hal_rcc_ex.h"
  ...
  */
    ...
    HAL_StatusTypeDef HAL_RCC_OscConfig(
        const RCC_OscInitTypeDef* pOscInitStruct);
    HAL_StatusTypeDef HAL_RCC_ClockConfig(
        const RCC_ClkInitTypeDef* pClkInitStruct, uint32_t FLatency);
