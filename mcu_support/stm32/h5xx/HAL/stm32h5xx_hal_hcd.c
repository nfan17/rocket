                                  ...
                                  uint8_t speed, uint8_t ep_type, uint16_t mps)
{
    ...
    if (used_channel == 0U)
    {
             ...
             (hhcd->Init.bulk_doublebuffer_enable == 1U)))
        {
            ...
            HCD_CLEAR_TX_DTOG(hhcd->Instance,
                              hhcd->hc[ch_num & 0xFU].phy_ch_num);

            /* Clear Channel DTOG RX */
            HCD_CLEAR_RX_DTOG(hhcd->Instance,
                              hhcd->hc[ch_num & 0xFU].phy_ch_num);
        }
        else
        {
            ...
            else
            {
                ...
                {
                    ep0_virtual_channel =
                        (uint8_t)(hhcd->ep0_PmaAllocState & 0xFU);
                        ...
                        (((hhcd->ep0_PmaAllocState & 0xF0U) >> 4) == CH_IN_DIR))
                    {
                        ...
                        {
                            status =
                                HAL_HCD_PMAlloc(hhcd, ch_num, HCD_SNG_BUF, 64U);
                ...
                else
                {
                    if (((hhcd->ep0_PmaAllocState & 0xF00U) >> 8) == 1U)
                    {
                        ...
                        if (((hhcd->ep0_PmaAllocState & 0xF0U) >> 4) ==
                            CH_IN_DIR)
                        {
...
void HAL_HCD_IRQHandler(HCD_HandleTypeDef* hhcd)
{
    ...
    if ((wIstr & USB_ISTR_CTR) == USB_ISTR_CTR)
    {
        ...
        {
            if ((HCD_GET_CHANNEL(hhcd->Instance, phy_chnum) & USB_CH_VTRX) !=
                0U)
            {
                ...
                else
                {
                    ...
                    epch_reg = (epch_reg & (USB_CHEP_REG_MASK &
                                            (~USB_CH_ERRRX) & (~USB_CH_VTRX))) |
            ...
            if ((HCD_GET_CHANNEL(hhcd->Instance, phy_chnum) & USB_CH_VTTX) !=
                0U)
            {
                /* Get Logical channel to check if the channel is already opened */
...
HAL_StatusTypeDef HAL_HCD_UnRegisterCallback(
    HCD_HandleTypeDef* hhcd, HAL_HCD_CallbackIDTypeDef CallbackID)
{
    HAL_StatusTypeDef status = HAL_OK;
                              ...
                              uint8_t phy_chnum, uint32_t regvalue)
{
    ...
    if ((regvalue & USB_CH_DTOG_TX) != 0U)
    {
        ...
        if (hhcd->hc[ch_num & 0xFU].xfer_len != 0U)
        {
                ...
                0U) /* Still data to fill in the buffer */
            {
                ...
                HCD_SET_CH_DBUF0_CNT(hhcd->Instance, phy_chnum, 1U,
                                     (uint16_t)len);
    ...
    else
    {
        /* Send Buffer1 */
        data_xfr =
            (uint16_t)(((USB_DRD_PMA_BUFF + phy_chnum)->RXBD & 0x03FF0000U) >>
                       16U);
        ...
        if (hhcd->hc[ch_num & 0xFU].xfer_len != 0U)
        {
                ...
                0U) /* Still data to fill in the buffer */
            {
                ...
                /* Write remaining data to Buffer0 */
                HCD_SET_CH_DBUF1_CNT(hhcd->Instance, phy_chnum, 1U,
                                     (uint16_t)len);
                             ...
                             uint8_t phy_chnum, uint32_t regvalue)
{
    ...
    {
        received_bytes =
            (uint16_t)HCD_GET_CH_DBUF0_CNT(hhcd->Instance, phy_chnum);
    ...
    else
    {
        /* Read from Buffer 1 */
        received_bytes =
            (uint16_t)HCD_GET_CH_DBUF1_CNT(hhcd->Instance, phy_chnum);
        ...
        else
        {
            ...
            if (((ch_reg)&USB_CH_RX_STRX) == USB_CH_RX_ACK_SBUF)
            {
                ...
                else
                {
                    ...
                    HCD_SET_CH_RX_STATUS(hhcd->Instance, phy_chnum,
                                         USB_CH_RX_STRX);
                }
        ...
        if ((ch_reg & USB_CH_UTYPE) == USB_EP_ISOCHRONOUS)
        {
            ...
            if ((hhcd->Instance->ISTR & USB_ISTR_ERR) == 0U)
            {
                ...
                else /* double buffer isochronous out */
                {
                    ...
                    USB_DRD_SET_CHEP_TX_STATUS(hhcd->Instance, phy_chnum,
                                               USB_CH_TX_DIS);
                }
#endif /* (USE_USB_DOUBLE_BUFFER == 1U) */
            ...
            /*TX COMPLETE*/
#if (USE_HAL_HCD_REGISTER_CALLBACKS == 1U)
            hhcd->HC_NotifyURBChangeCallback(hhcd, (uint8_t)chnum,
                                             hhcd->hc[chnum & 0xFU].urb_state);
#else
            HAL_HCD_HC_NotifyURBChange_Callback(
                hhcd, (uint8_t)chnum, hhcd->hc[chnum & 0xFU].urb_state);
#endif /* USE_HAL_HCD_REGISTER_CALLBACKS */
        }
        else /* Manage all Non Isochronous Transaction */
        {
            ...
            if ((ch_reg & USB_CH_TX_STTX) == USB_CH_TX_ACK_SBUF)
            {
                ...
                if (hhcd->hc[chnum & 0xFU].xfer_len != 0U)
                {
                    ...
                    /* start a new transfer */
                    (void)USB_HC_StartXfer(hhcd->Instance,
                                           &hhcd->hc[chnum & 0xFU]);
                }
                else
                {
            ...
            else if ((ch_reg & USB_CH_TX_STTX) == USB_CH_TX_ACK_DBUF)
            {
                ...
                (void)HCD_HC_OUT_BulkDb(hhcd, chnum, (uint8_t)phy_chnum,
                                        ch_reg);
            }
#endif /* (USE_USB_DOUBLE_BUFFER == 1U) */
            else
                                          ...
                                          uint8_t phy_chnum, uint8_t dir)
{
    ...
    else
    {
        ...
        {
            return (
                (uint8_t)((hhcd->phy_chin_state[phy_chnum & 0x7U] & 0x00F0U) >>
                          4U) -
                1U);
        }
        else
