    ...
    if (hirda->gState == HAL_IRDA_STATE_READY)
    {
       ...
       should be aligned on a u16 frontier, so nbByte should be equal to Size multiplied by 2 */
        ...
        else
        {
            /* Enable the IRDA transmit DMA channel */
            status =
                HAL_DMA_Start_IT(hirda->hdmatx, (uint32_t)hirda->pTxBuffPtr,
    ...
    if (hirda->RxState == HAL_IRDA_STATE_READY)
    {
       ...
       should be aligned on a u16 frontier, so nbByte should be equal to Size multiplied by 2 */
        ...
        else
        {
            /* Enable the DMA channel */
            status =
                HAL_DMA_Start_IT(hirda->hdmarx, (uint32_t)&hirda->Instance->RDR,
    ...
    switch (clocksource)
    {
        case IRDA_CLOCKSOURCE_PLL2Q:
            ...
            tmpreg = (uint32_t)(IRDA_DIV_SAMPLING16(
                pll2_clocks.PLL2_Q_Frequency, hirda->Init.BaudRate,
                hirda->Init.ClockPrescaler));
            break;
#if defined(RCC_CR_PLL3ON)
        case IRDA_CLOCKSOURCE_PLL3Q:
            ...
            tmpreg = (uint32_t)(IRDA_DIV_SAMPLING16(
                pll3_clocks.PLL3_Q_Frequency, hirda->Init.BaudRate,
                hirda->Init.ClockPrescaler));
            break;
#endif /* RCC_CR_PLL3ON */
...
static HAL_StatusTypeDef IRDA_CheckIdleState(IRDA_HandleTypeDef* hirda)
{
    ...
    if ((hirda->Instance->CR1 & USART_CR1_TE) == USART_CR1_TE)
    {
        ...
        if (IRDA_WaitOnFlagUntilTimeout(hirda, USART_ISR_TEACK, RESET,
                                        tickstart,
    ...
    if ((hirda->Instance->CR1 & USART_CR1_RE) == USART_CR1_RE)
    {
        ...
        if (IRDA_WaitOnFlagUntilTimeout(hirda, USART_ISR_REACK, RESET,
                                        tickstart,
                                        IRDA_TEACK_REACK_TIMEOUT) != HAL_OK)
        {
            /* Timeout occurred */
...
static void IRDA_Transmit_IT(IRDA_HandleTypeDef* hirda)
{
    ...
    if (hirda->gState == HAL_IRDA_STATE_BUSY_TX)
    {
        ...
        else
        {
            ...
            {
                tmp =
                    (const uint16_t*)hirda->pTxBuffPtr; /* Derogation R.11.3 */
