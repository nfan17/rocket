...
#define IS_EXTI_LINE_ATTRIBUTES(__ATTRIBUTES__)                     \
  ...
  */
    ...
    uint32_t HAL_EXTI_GetPending(const EXTI_HandleTypeDef* hexti,
                                 uint32_t Edge);
  ...
  */
    ...
    void HAL_EXTI_ConfigLineAttributes(uint32_t ExtiLine,
                                       uint32_t LineAttributes);
    HAL_StatusTypeDef HAL_EXTI_GetConfigLineAttributes(
        uint32_t ExtiLine, uint32_t* pLineAttributes);

#if defined(__ARM_FEATURE_CMSE) && (__ARM_FEATURE_CMSE == 3U)
    HAL_StatusTypeDef HAL_EXTI_LockConfigAttributes(void);
    HAL_StatusTypeDef HAL_EXTI_GetLockConfigAttributes(
