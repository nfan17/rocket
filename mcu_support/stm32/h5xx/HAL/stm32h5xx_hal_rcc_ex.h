...
#include "stm32h5xx_hal_def.h"
  ...
  */
    typedef struct
    {
        ...
        uint32_t
            PLL2FRACN; /*!<PLL2FRACN: Specifies Fractional Part Of The Multiplication Factor for
                            PLL2 VCO It should be a value between 0 and 8191                           */

        uint32_t
            PLL2ClockOut; /*!< PLL2ClockOut: specifies PLL2 output clock to be enabled.
  ...
  */
    typedef struct
    {
        ...
        uint32_t
            PLL3FRACN; /*!<PLL3FRACN: Specifies Fractional Part Of The Multiplication Factor for
                            PLL3 VCO It should be a value between 0 and 8191                           */

        uint32_t
            PLL3ClockOut; /*!< PLL3ClockOut: specifies PLL3 output clock to be enabled.
  ...
  */
    typedef struct
    {
        ...
        uint32_t
            AdcDacClockSelection; /*!< Specifies ADC1, ADC2 and DAC interface clock source.
                                        This parameter can be a value of @ref RCCEx_ADCDAC_Clock_Source */

        uint32_t
            DacLowPowerClockSelection; /*!< Specifies DAC low-power clock source used in Sample and hold mode.
  ...
  */
    typedef struct
    {
        ...
        uint32_t
            ReloadValue; /*!< Specifies the value to be loaded in the frequency error counter with each SYNC
...
#define RCC_CRS_FREQERRORDIR_UP \
    ((uint32_t)0x00000000U) /*!< Upcounting direction, the actual frequency is above the target */
...
#define __HAL_RCC_TIMIC_ENABLE() \
    SET_BIT(                     \
        RCC->CCIPR1,             \
        RCC_CCIPR1_TIMICSEL) /*!< HSI/1024, CSI/128 and HSI/8 generation for Timers 12,15 and LPTimer2 Input capture */
...
#define __HAL_RCC_FDCAN_CONFIG(__FDCAN_CLKSOURCE__) \
    MODIFY_REG(RCC->CCIPR5, RCC_CCIPR5_FDCANSEL,    \
...
#define __HAL_RCC_UART5_CONFIG(__UART5_CLKSOURCE__) \
    MODIFY_REG(RCC->CCIPR1, RCC_CCIPR1_UART5SEL,    \
...
#define __HAL_RCC_UART7_CONFIG(__UART7_CLKSOURCE__) \
    MODIFY_REG(RCC->CCIPR1, RCC_CCIPR1_UART7SEL,    \
...
#define __HAL_RCC_UART8_CONFIG(__UART8_CLKSOURCE__) \
    MODIFY_REG(RCC->CCIPR1, RCC_CCIPR1_UART8SEL,    \
...
#define __HAL_RCC_UART9_CONFIG(__UART9_CLKSOURCE__) \
    MODIFY_REG(RCC->CCIPR1, RCC_CCIPR1_UART9SEL,    \
               (uint32_t)(__UART9_CLKSOURCE__))
...
#define __HAL_RCC_OSPI_CONFIG(__OSPI_CLKSOURCE__)  \
    MODIFY_REG(RCC->CCIPR4, RCC_CCIPR4_OCTOSPISEL, \
               (uint32_t)(__OSPI_CLKSOURCE__))
...
#define IS_RCC_PLL2_VCORGE_VALUE(VALUE)     \
    (((VALUE) == RCC_PLL2_VCORANGE_WIDE) || \
     ((VALUE) == RCC_PLL2_VCORANGE_MEDIUM))
...
#define IS_RCC_PLL3_VCORGE_VALUE(VALUE)     \
    (((VALUE) == RCC_PLL3_VCORANGE_WIDE) || \
     ((VALUE) == RCC_PLL3_VCORANGE_MEDIUM))
...
#define IS_RCC_CRS_FREQERRORDIR(__DIR__)       \
  ...
  */
    ...
    HAL_StatusTypeDef HAL_RCCEx_PeriphCLKConfig(
        const RCC_PeriphCLKInitTypeDef* pPeriphClkInit);
