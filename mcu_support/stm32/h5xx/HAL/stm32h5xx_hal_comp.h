...
/** @addtogroup STM32H5xx_HAL_Driver
  ...
  */
    ...
    {
        uint32_t
            Mode; /*!< Set comparator operating mode to adjust power and speed.
        ...
        uint32_t
            TriggerMode; /*!< Set the comparator output triggering External Interrupt Line (EXTI).
...
#if (USE_HAL_COMP_REGISTER_CALLBACKS == 1U)
        void (*TriggerCallback)(struct __COMP_HandleTypeDef*
                                    hcomp); /*!< COMP trigger callback    */
        void (*MspInitCallback)(struct __COMP_HandleTypeDef*
                                    hcomp); /*!< COMP Msp Init callback   */
...
#define __HAL_COMP_IS_LOCKED(__HANDLE__)                         \
    (READ_BIT((__HANDLE__)->Instance->CFGR1, COMP_CFGR1_LOCK) == \
     COMP_CFGR1_LOCK)
