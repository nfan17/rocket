                                              ...
                                              uint32_t BurstLength)
{
    ...
    if (hdma != NULL)
    {
        ...
        if (((hdma->Mode & DMA_LINKEDLIST) == DMA_LINKEDLIST) &&
            (hdma->LinkedListQueue != 0U) &&
            (hdma->LinkedListQueue->Head != 0U))
        {
            data_width = hdma->LinkedListQueue->Head->LinkRegisters[0] &
                         DMA_CTR1_SDW_LOG2;
        }
        else
        {
        ...
        switch (data_width)
        {
            ...
            {
                BlockDataLength =
                    ((BurstLength >> TIM_DCR_DBL_Pos) + 1UL) * 2UL;
                break;
            }
            case DMA_SRC_DATAWIDTH_WORD:
            {
                BlockDataLength =
                    ((BurstLength >> TIM_DCR_DBL_Pos) + 1UL) * 4UL;
                break;
            }
            default:
                status = HAL_ERROR;
        ...
        if (status == HAL_OK)
        {
            status = HAL_TIM_DMABurst_MultiWriteStart(
                htim, BurstBaseAddress, BurstRequestSrc, BurstBuffer,
                                   ...
                                   uint32_t dst, uint32_t length)
{
    ...
    if ((hdma->Mode & DMA_LINKEDLIST) == DMA_LINKEDLIST)
    {
            ...
            (hdma->LinkedListQueue->Head != 0U))
        {
            ...
            hdma->LinkedListQueue->Head
                ->LinkRegisters[NODE_CSAR_DEFAULT_OFFSET] = src;
            hdma->LinkedListQueue->Head
                ->LinkRegisters[NODE_CDAR_DEFAULT_OFFSET] = dst;
