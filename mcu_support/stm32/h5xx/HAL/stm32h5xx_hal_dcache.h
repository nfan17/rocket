...
#include "stm32h5xx_hal_def.h"
  ...
  */
    typedef enum
    {
        ...
        HAL_DCACHE_STATE_TIMEOUT =
            0x05U, /*!< Timeout state                              */
        HAL_DCACHE_STATE_ERROR =
            0x06U, /*!< DCACHE state error                         */
  ...
  */
    typedef struct __DCACHE_HandleTypeDef
    {
        ...
        void (*InvalidateByAddrCallback)(
            struct __DCACHE_HandleTypeDef* hdcache);
        void (*InvalidateCompleteCallback)(
            struct __DCACHE_HandleTypeDef* hdcache);
  ...
  */
    typedef enum
    {
        ...
        HAL_DCACHE_ERROR_CB_ID =
            0x04U, /*!< DCACHE Error callback ID                           */

        HAL_DCACHE_MSPINIT_CB_ID =
            0x05U, /*!< DCACHE Msp Init callback ID                        */
        HAL_DCACHE_MSPDEINIT_CB_ID =
            0x06U /*!< DCACHE Msp DeInit callback ID                      */
...
#define __HAL_DCACHE_GET_IT_SOURCE(__HANDLE__, __INTERRUPT__)   \
    ((READ_BIT((__HANDLE__)->Instance->IER, (__INTERRUPT__)) == \
      (__INTERRUPT__))                                          \
         ? SET                                                  \
         : RESET)
...
#define __HAL_DCACHE_CLEAR_FLAG(__HANDLE__, __FLAG__) \
  ...
  */
    ...
    HAL_StatusTypeDef HAL_DCACHE_CleanInvalidByAddr(
        DCACHE_HandleTypeDef* hdcache, const uint32_t* const pAddr,
        uint32_t dSize);
    ...
    void HAL_DCACHE_CleanAndInvalidateByAddrCallback(
        DCACHE_HandleTypeDef* hdcache);

    /* Callbacks Register/UnRegister functions ***/
    HAL_StatusTypeDef HAL_DCACHE_RegisterCallback(
        DCACHE_HandleTypeDef* hdcache, HAL_DCACHE_CallbackIDTypeDef CallbackID,
    ...
    uint32_t HAL_DCACHE_Monitor_GetReadHitValue(
        const DCACHE_HandleTypeDef* hdcache);
    uint32_t HAL_DCACHE_Monitor_GetReadMissValue(
        const DCACHE_HandleTypeDef* hdcache);
    uint32_t HAL_DCACHE_Monitor_GetWriteHitValue(
        const DCACHE_HandleTypeDef* hdcache);
    uint32_t HAL_DCACHE_Monitor_GetWriteMissValue(
        const DCACHE_HandleTypeDef* hdcache);
  ...
  * @{
  */
    HAL_DCACHE_StateTypeDef HAL_DCACHE_GetState(
        const DCACHE_HandleTypeDef* hdcache);
