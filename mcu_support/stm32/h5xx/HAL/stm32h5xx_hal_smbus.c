...
static HAL_StatusTypeDef SMBUS_WaitOnFlagUntilTimeout(
    SMBUS_HandleTypeDef* hsmbus, uint32_t Flag, FlagStatus Status,
    uint32_t Timeout);
    ...
    if (hsmbus->State == HAL_SMBUS_STATE_READY)
    {
            ...
            (hsmbus->XferSize == MAX_NBYTE_SIZE))
        {
            SMBUS_TransferConfig(
                hsmbus, DevAddress, (uint8_t)hsmbus->XferSize,
                                              ...
                                              uint32_t XferOptions)
{
    ...
    if ((hsmbus->State & HAL_SMBUS_STATE_LISTEN) == HAL_SMBUS_STATE_LISTEN)
    {
        ...
        {
            SMBUS_TransferConfig(
                hsmbus, 0, (uint8_t)hsmbus->XferSize,
                SMBUS_RELOAD_MODE | (hsmbus->XferOptions & SMBUS_SENDPEC_MODE),
                SMBUS_NO_STARTSTOP);
        }
        else
    ...
    if (hsmbus->State == HAL_SMBUS_STATE_READY)
    {
        ...
        do
        {
            ...
            if (__HAL_SMBUS_GET_FLAG(hsmbus, SMBUS_FLAG_AF) == RESET)
            {
                ...
                if (SMBUS_WaitOnFlagUntilTimeout(hsmbus, SMBUS_FLAG_STOPF,
