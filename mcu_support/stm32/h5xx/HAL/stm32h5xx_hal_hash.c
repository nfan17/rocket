    ...
    HASH_HandleTypeDef* hhash, HAL_HASH_CallbackIDTypeDef CallbackID)
{
    ...
    if (hhash->State == HAL_HASH_STATE_READY)
    {
        switch (CallbackID)
        {
            ...
            case HAL_HASH_DGSTCPLT_CB_ID:
                hhash->DgstCpltCallback =
                    HAL_HASH_DgstCpltCallback; /* Legacy weak digest computation
      completion callback */
                break;
                                 ...
                                 uint8_t* const pOutBuffer, uint32_t Timeout)
{
    ...
    if (hhash->State == HAL_HASH_STATE_READY)
    {
        ...
        if (HASH_WaitOnFlagUntilTimeout(hhash, HASH_FLAG_DCIS, RESET,
                                        Timeout) != HAL_OK)
        {
            return HAL_ERROR;
        }
      ...
      computation. */
        ...
        if ((hhash->hdmain->Mode & DMA_LINKEDLIST) == DMA_LINKEDLIST)
        {
                ...
                (hhash->hdmain->LinkedListQueue->Head != NULL))
            {
                ...
                hhash->hdmain->LinkedListQueue->Head
                    ...
                    (uint32_t)(hhash
                                   ->pHashInBuffPtr); /* Set DMA source address */
                hhash->hdmain->LinkedListQueue->Head
                    ->LinkRegisters[NODE_CDAR_DEFAULT_OFFSET] =
                    (uint32_t)&hhash->Instance
                        ->DIN; /* Set DMA destination address */
        ...
        else
        {
            status = HAL_DMA_Start_IT(
                hhash->hdmain, (uint32_t)pInBuffer,
...
HAL_StatusTypeDef HAL_HASH_HMAC_AccumulateLast_IT(
    HASH_HandleTypeDef* hhash, const uint8_t* const pInBuffer, uint32_t Size,
    uint8_t* const pOutBuffer)
{
    HAL_StatusTypeDef status;
                                          ...
                                          uint8_t* const pOutBuffer)
{
    ...
    {
        status = HAL_DMA_Start_IT(
            hhash->hdmain, (uint32_t)(hhash->pHashInBuffPtr),
            (uint32_t)&hhash->Instance->DIN,
            ((((hhash->Size) % 4U) != 0U)
                 ? ((hhash->Size) + (4U - ((hhash->Size) % 4U)))
                 : ((hhash->Size))));
...
static HAL_StatusTypeDef HASH_WriteData_IT(HASH_HandleTypeDef* hhash)
{
    ...
    uint32_t nbbytePartialHash = (((hhash->Instance->SR) >> 16U) *
                                  4U); /* Nb byte  to enter in HASH fifo to trig
            ...
            else
            {
                ...
                if (((hhash->HashInCount) + nbbytePartialHash) < (hhash->Size))
                {
                    ...
                    do
                    {
                        ...
                        HAL_IS_BIT_CLR(hhash->Instance->SR, HASH_FLAG_DINIS));
                }
                else
                {
                ...
                HAL_HASH_InCpltCallback(hhash);
                    ...
                    if (hhash->Accumulation == 0U)
                    {
                        if (__HAL_HASH_GET_IT_SOURCE(hhash, HASH_IT_DINI))
                        {
                            ...
                            } while (HAL_IS_BIT_CLR(hhash->Instance->SR,
                                                    HASH_FLAG_DCIS));
                        }
                    }
                    else
                    ...
                    HAL_HASH_InCpltCallback(hhash);
                        ...
                        if (hhash->Accumulation == 0U)
                        {
                            if (__HAL_HASH_GET_IT_SOURCE(hhash, HASH_IT_DINI))
                            {
                                ...
                                } while (HAL_IS_BIT_SET(hhash->Instance->SR,
                                                        HASH_FLAG_BUSY));
                     ...
                     HAL_HASH_PHASE_HMAC_STEP_3) /* loading Key*/
            {
