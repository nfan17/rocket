...
#include "stm32h5xx_hal_def.h"
  ...
  */
    typedef enum
    {
        ...
        HAL_FMAC_STATE_BUSY =
            0x24U, /*!< FMAC internal process is ongoing                               */
        ...
        HAL_FMAC_STATE_TIMEOUT =
            0xA0U, /*!< FMAC in Timeout state                                          */
        HAL_FMAC_STATE_ERROR =
            0xE0U /*!< FMAC in Error state                                            */
...
/**
  ...
  */
...
#endif /* USE_HAL_FMAC_REGISTER_CALLBACKS */
    {
        ...
        uint8_t
            InputAccess; /*!< Access to the input buffer (internal memory area):
                                                     DMA, IT, Polling, None.
                                                     This parameter can be a value of @ref FMAC_Buffer_Access. */

        uint8_t
            OutputAccess; /*!< Access to the output buffer (internal memory area):
                                                     DMA, IT, Polling, None.
                                                     This parameter can be a value of @ref FMAC_Buffer_Access. */
        ...
        uint16_t
            InputCurrentSize; /*!< Number of the input elements already written into FMAC */

        uint16_t*
            pInputSize; /*!< Number of input elements to write (memory allocated to pInput).
        ...
        uint16_t
            OutputCurrentSize; /*!< Number of the output elements already read from FMAC */

        uint16_t*
            pOutputSize; /*!< Number of output elements to read (memory allocated to pOutput).
                                                      In case of early interruption of the filter operation,
                                                      its value will be updated. */
        ...
        DMA_HandleTypeDef*
            hdmaPreload; /*!< FMAC peripheral preloaded data (X1, X2 and Y) DMA handle
                                                      parameters */

#if (USE_HAL_FMAC_REGISTER_CALLBACKS == 1)
        void (*ErrorCallback)(
            struct __FMAC_HandleTypeDef*
                hfmac); /*!< FMAC error callback                  */
  ...
  */
    typedef enum
    {
        ...
        HAL_FMAC_MSPINIT_CB_ID =
            0x07U, /*!< FMAC MspInit callback ID                */
        HAL_FMAC_MSPDEINIT_CB_ID =
            0x08U, /*!< FMAC MspDeInit callback ID              */
  ...
  */
    ...
    {
        uint8_t
            InputBaseAddress; /*!< Base address of the input buffer (X1) within the internal memory
        ...
        uint32_t
            InputThreshold; /*!< Input threshold: the buffer full flag will be set if the number
                                                     of free spaces in the buffer is lower than this threshold.
                                                     This parameter can be a value
                                                     of @ref FMAC_Data_Buffer_Threshold. */

        uint8_t
            CoeffBaseAddress; /*!< Base address of the coefficient buffer (X2) within the internal
                                                     memory (0x00 to 0xFF). Ignored if CoeffBufferSize is set to 0
                                                     (previous configuration kept).
                                                     Note: the buffers can overlap or even coincide exactly. */

        uint8_t
            CoeffBufferSize; /*!< Number of 16-bit words allocated to the coefficient buffer.
                                                     0 if a previous configuration should be kept. */

        uint8_t
            OutputBaseAddress; /*!< Base address of the output buffer (Y) within the internal
                                                     memory (0x00 to 0xFF). Ignored if OuputBufferSize is set to 0
                                                     (previous configuration kept).
                                                     Note: the buffers can overlap or even coincide exactly. */

        uint8_t
            OutputBufferSize; /*!< Number of 16-bit words allocated to the output buffer
                                                    (including the optional "headroom").
                                                     0 if a previous configuration should be kept. */

        uint32_t
            OutputThreshold; /*!< Output threshold: the buffer empty flag will be set if the number
                                                     of unread values in the buffer is lower than this threshold.
                                                     This parameter can be a value
                                                     of @ref FMAC_Data_Buffer_Threshold. */

        int16_t*
            pCoeffA; /*!< [IIR only] Initialization of the coefficient vector A.
        ...
        uint32_t
            Clip; /*!< Enable or disable the clipping feature. If the q1.15 range
...
#define IS_FMAC_THRESHOLD_APPLICABLE(__SIZE__, __WM__, __ACCESS__) \
  ...
  */
    ...
    HAL_StatusTypeDef HAL_FMAC_FilterConfig_DMA(
        FMAC_HandleTypeDef* hfmac, FMAC_FilterConfigTypeDef* pConfig);
