      ...
      */
                ...
                (Size >= husart->NbRxDataToProcess))
            {
                for (nb_dummy_data = husart->NbRxDataToProcess;
                     nb_dummy_data > 0U; nb_dummy_data--)
                {
                    husart->Instance->TDR =
        ...
        if (husart->hdmatx != NULL)
        {
         ...
         should be aligned on a u16 frontier, so nbByte should be equal to Size multiplied by 2 */
            ...
            else
            {
                /* Enable the USART transmit DMA channel */
                status =
                    HAL_DMA_Start_IT(husart->hdmatx, *(const uint32_t*)tmp,
            ...
            if (status == HAL_OK)
            {
                ...
                else
                {
                    status = HAL_DMA_Start_IT(
                        husart->hdmatx, *(const uint32_t*)tmp,
                        (uint32_t)&husart->Instance->TDR, nbByte);
                }
            }
        }
   ...
   * - USART Clock Prescaler : set PRESCALER according to husart->Init.ClockPrescaler value */
    ...
    switch (clocksource)
    {
        ...
        case USART_CLOCKSOURCE_PLL2Q:
            ...
            usartdiv = (uint32_t)(USART_DIV_SAMPLING8(
                pll2_clocks.PLL2_Q_Frequency, husart->Init.BaudRate,
                husart->Init.ClockPrescaler));
            break;
#if defined(RCC_CR_PLL3ON)
        case USART_CLOCKSOURCE_PLL3Q:
            ...
            usartdiv = (uint32_t)(USART_DIV_SAMPLING8(
                pll3_clocks.PLL3_Q_Frequency, husart->Init.BaudRate,
                husart->Init.ClockPrescaler));
            break;
#endif /* RCC_CR_PLL3ON */
...
static HAL_StatusTypeDef USART_CheckIdleState(USART_HandleTypeDef* husart)
{
    ...
    if ((husart->Instance->CR1 & USART_CR1_TE) == USART_CR1_TE)
    {
        ...
        if (USART_WaitOnFlagUntilTimeout(husart, USART_ISR_TEACK, RESET,
                                         tickstart,
    ...
    if ((husart->Instance->CR1 & USART_CR1_RE) == USART_CR1_RE)
    {
        ...
        if (USART_WaitOnFlagUntilTimeout(husart, USART_ISR_REACK, RESET,
                                         tickstart,
                                         USART_TEACK_REACK_TIMEOUT) != HAL_OK)
        {
            /* Timeout occurred */
