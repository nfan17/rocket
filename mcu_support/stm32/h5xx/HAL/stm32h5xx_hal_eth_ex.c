    ...
    ETH_L4FilterConfigTypeDef* pL4FilterConfig)
{
    ...
    MODIFY_REG(*configreg, (ETH_MACL4AR_L4DP | ETH_MACL4AR_L4SP),
    ...
    ETH_L4FilterConfigTypeDef* pL4FilterConfig)
{
    ...
    pL4FilterConfig->DestinationPort =
        (READ_BIT(*((__IO uint32_t*)(&(heth->Instance->MACL4A0R) + Filter)),
                  ETH_MACL4AR_L4DP) >>
         16);
    pL4FilterConfig->SourcePort =
        READ_BIT(*((__IO uint32_t*)(&(heth->Instance->MACL4A0R) + Filter)),
    ...
    if (VLANTag == ETH_INNER_TX_VLANTAG)
    {
        ...
        pVlanConfig->SVLANType =
            ((READ_BIT(heth->Instance->MACIVIR, ETH_MACVIR_CSVL) >> 19) == 0U)
                ? DISABLE
                : ENABLE;
        pVlanConfig->VLANTagControl = READ_BIT(
            heth->Instance->MACIVIR, (ETH_MACVIR_VLP | ETH_MACVIR_VLC));
    }
    else
    {
        pVlanConfig->SourceTxDesc =
