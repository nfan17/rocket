...
static void Ads111x_Wait_Complete(Ads111x* dev)
{
    ...
    {
        dev->status = dev->bus->read(dev->bus, ADS_REG_CONFIG_PTR, rxdata,
                                     ADS_CONFIG_BUFFER_SIZE);
    }
}
...
void Ads111x_Start_Sample(Ads111x* dev, uint8_t channel)
{
    ...
    configs[0] =
        ADS_CONFIG_UPPER_BASE | ADS_CONFIG_OS | dev->mode | dev->gain | channel;
     ...
     */
    dev->status = dev->bus->write(dev->bus, ADS_REG_CONFIG_PTR, configs,
                                  ADS_CONFIG_BUFFER_SIZE);
}
...
int16_t Ads111x_Read_Sample(Ads111x* dev)
{
     ...
     */
    uint8_t rxdata[2] = {0};
    dev->status = dev->bus->read(dev->bus, ADS_REG_CONVERSION_PTR, rxdata,
                                 ADS_CONFIG_BUFFER_SIZE);
